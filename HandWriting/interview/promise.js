...
class APromise {
    constructor(executor) {
        if (typeof executor !== 'function') {
            throw new TypeError(
                `Promise resolver ${executor.toString()} is not a function`
            );
        }
        this.status = PENDING;
        this.listeners = {
                ...
                try {
                    ...
                    if (isThenable(result)) {
                        ...
                        if (isPromise(result)) {
                            ...
                            new this.constructor(result.then).then(
                                successHandler,
                                errorHandler
                            );
                        }
...
APromise.resolve = data => {
    ...
    return isThenable(data)
        ? new APromise(data.then)
        : new APromise((resolve, reject) => resolve(data));
};
